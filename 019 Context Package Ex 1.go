...
func findUser(rw http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		if ip, ok := ctx.Value(userIPKey).(key); ok {
		...
		log.Println("DB gorouting is terminating")
	}()
	...
	case d := <-c:
		...
		// as the status code.
		sendResponse(rw, d, http.StatusOK)

		// Log we sent the response with a StatusOk
		log.Println("Send response with StatusOK")
	...
	case  <- ctx.Done():
		...
		log.Println("Response with StatusRequestTimeout")
