...
func (e *appError) Error() string {
	return fmt.Sprintf("Error: %s Message: %s (Code: %d)", e.err, e.message, e.code)
}
...
func checkFlag(value bool /* parameter */) error /* return arg */ {
	// If the parameter is false return an appError.
	if !value {
		return &appError{errors.New("Flag false"), "The flag is false.", 9}
	}

	// Return a default error.
	return errors.New("The flag is true")
}
